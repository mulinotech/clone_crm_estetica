...
export default function App() {
  ...
  const [isCrmOpen, setIsCrmOpen] = useState<boolean>(() => {
    return localStorage.getItem("crm_estetica_open") === "true";
  });
  const [presetQuizTreatmentName, setPresetQuizTreatmentName] = useState<string>("");
  const [activeReviewIndex, setActiveReviewIndex] = useState<number>(0);

  useEffect(() => {
    localStorage.setItem("crm_estetica_open", String(isCrmOpen));
  }, [isCrmOpen]);
  ...
  useEffect(() => {
    fetch(API_URL)
      ...
      .catch(err => {
        ...
        const saved = localStorage.getItem("crm_estetica_leads_v2");
        if (saved) {
          setLeads(JSON.parse(saved));
        } else {
          localStorage.setItem("crm_estetica_leads_v2", JSON.stringify(PRESEEDED_LEADS));
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = ["inicio", "tratamentos", "dra-nathali", "resultados", "quiz", "contato"];
  ...
  const handleLeadCapture = (newLead: Lead) => {
    ...
    setLeads(updated);
    localStorage.setItem("crm_estetica_leads_v2", JSON.stringify(updated));
  ...
  const handleUpdateLeadStatus = (leadId: string, newStatus: Lead["status"], phone?: string, email?: string) => {
    ...
    .then(res => {
      if (!res.ok) console.error("Erro ao atualizar o lead no backend.");
    })
    .catch(err => console.error("Falha ao conectar com o backend:", err));
  };
  ...
  const handleDeleteLead = (leadId: string) => {
    ...
    .then(res => {
      if (!res.ok) console.error("Erro ao deletar lead do backend.");
    })
    .catch(err => console.error("Falha ao conectar com o backend:", err));
  };
  ...
  const handleClearLeads = () => {
    if (window.confirm("Deseja realmente limpar todos os leads capturados neste navegador?")) {
      setLeads([]);
      localStorage.removeItem("crm_estetica_leads_v2");
      ...
      <footer className="bg-bg-luxe border-t border-primary/20 py-16 text-neutral-muted text-xs text-left relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
            ...
            <div className="md:col-span-4 space-y-4">
              ...
              <p className="text-neutral-muted font-light leading-relaxed max-w-sm">
                Uma experiência estética integrativa de alto escalão. 
                Sua beleza natural sob cuidados médicos e científicos padrão ouro.
              </p>
              ...
              <div className="flex items-center space-x-3.5 pt-2">
                ...
                <span className="text-[11px] text-neutral-muted font-semibold tracking-wider">Siga-nos nas redes sociais</span>
              </div>
            </div>
            ...
            <div className="md:col-span-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-neutral-muted">
                ...
                <button onClick={() => handleNavigation("dra-nathali")} className="hover:text-primary transition-colors text-left font-semibold py-1 cursor-pointer">Corpo Clínico</button>
              ...
              <div className="pt-2 border-t border-primary/10">
                <span className="text-[9px] text-neutral-muted/70 leading-relaxed font-light block">
                  Responsável Técnica • Harmonização Estética Avançada • Registro de Classe Ativo • Consultórios devidamente regulamentados.
                </span>
              </div>
            </div>
          ...
          <div className="border-t border-primary/10 mt-12 pt-8 flex flex-col sm:flex-row items-center justify-between text-[10px] text-neutral-muted space-y-2.5 sm:space-y-0">
            <p>© {new Date().getFullYear()} CRM Estética SaaS. Todos os direitos reservados.</p>
