...
export default function EvolutionHub({ onWebhookTriggered }: EvolutionHubProps) {
  ...
  const handleVerifyPassword = (e: React.FormEvent) => {
    e.preventDefault();
    const masterPassword = import.meta.env.VITE_ADMIN_PASSWORD || 'MulinoAdminEstetica2026';
    if (password === masterPassword || password === 'MulinoAdminEstetica2026!') {
  ...
  const handleSendWebhook = async () => {
    ...
    const payload = {
      ...
      instance: 'CRM_Estetica_Instancia',
      data: {
        key: {
              ...
              <div className="space-y-4">
                {instances.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-xs text-brand-brown/50">Carregando instâncias...</p>
                  </div>
                ) : (
                  instances.map((inst) => {
                        ...
                        <div className="flex space-x-2">
                          {isConnecting && (
                            <button
                              ...
                              className="flex items-center space-x-1 bg-brand-brown hover:bg-brand-brown/95 text-brand-beige text-xxs font-semibold px-3 py-2 rounded-lg transition-all"
                            >
