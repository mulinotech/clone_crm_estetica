...
export default function LeadDashboard({
  ...
  const handleLogin = (e: FormEvent) => {
    e.preventDefault();
    const adminPassword = import.meta.env.VITE_ADMIN_PASSWORD || "MulinoAdminEstetica2026";
  ...
  const handleExportJSON = () => {
    ...
    downloadAnchor.setAttribute("download", `leads_crm_estetica_${new Date().toISOString().split('T')[0]}.json`);
  ...
  const handleSendWhatsAppReach = (lead: Lead) => {
    ...
    const defaultTemplate = `Olá ${lead.name}! Sou a concierge de atendimento da nossa clínica de estética. Recebemos seu interesse em nosso protocolo de ${lead.treatment} e gostaria de agendar uma avaliação VIP com nossa equipe médica especializada. Qual o melhor dia e horário para você?`;
